...
Drone.prototype.start = function (app, callback) {
  ...
  this.spawner.trySpawn(app, function (err, result) {
    ...
    console.error(app.hash, app.id)
    result.hash = app.hash
    result.id = app.id
    result.uid = '#' + Math.random() + Math.random() + Math.random()
...
Drone.prototype.stop = function (name, cleanup, callback) {
  ...
  async.map(keys, function (pid, next) {self.stopInstance(name, pid, cleanup, next)}, callback);
};

//
// ### function stopInstance (name, callback)
// #### @name {string} Name of the application to stop (i.e. app.name).
// #### @pid {string} pid to stop.
// #### @cleanup {bool} (optional) Remove all autostart files (default=true).
// #### @callback {function} Continuation passed to respond to.
// Stops the drone with app.name === name and pid.
// used for zero downtime update.
Drone.prototype.stopInstance = function (appName, pid, cleanup, callback) {
    var app = this.apps[appName]

    app.drones[pid].monitor.stop();
    var data = app.drones[pid].process
    this._remove(app, app.drones[pid], cleanup, function () {
      callback(null, data);
    });
...
// Stops the drone with app.name === name and pid.
// used for zero downtime update.
Drone.prototype.stopOnPort = function (appName, port, cleanup, callback) {
  if(!callback) callback = cleanup, cleanup = true
  var app = this.apps[appName]

  if(!app)
    return callback(new Error('no app: ' + JSON.stringify(appName)))

  //get the pid of the app on given port
  //it would all be simpler if drones where indexed by port.
  var drones = Object.keys(app.drones).map(function (pid) {return app.drones[pid]})

  for (pid in drones) {
    var drone = drones[pid]
    if(port == drone.socket.port) {
      drone.monitor.stop();
      var data = drone.process
      this._remove(app, drone, cleanup, function () {
        callback(null, data);
      });
      return
    }
  }
  callback(new Error('no drones running on port:' + port));
