...
/*
    ...
    // stop instance
    //
    this.post(/\/([\w\-\.]+)\/\/([\w\-\.]+)\/stop_instance/).bind(function (response, id, pid) {
      drone.stop(id, pid, function (err, result) {
        if (err || !result) {
    ...
    // stop instance
    //
    this.post(/\/([\w\-\.]+)\/stop\/([\w\-\.]+)/).bind(function (response, id, port) {
      drone.stopOnPort(id, port, function (err, result) {
        if (err || !result) {
          err = err || new Error('Unknown error from drone.');
          haibu.emit('error:service', 'error', err);
          return response.send(500, {}, { error: err });
